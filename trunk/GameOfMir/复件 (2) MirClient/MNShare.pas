...
function GetMissionFalgStatus(nFlag: Integer): Integer;
    ...
    if s10 <> '' then begin
      if (pos('/', s10) <= 0) and (s10[1] = '^') then begin
        ...
        if CompareLStr(sLabel, VAR_FLAG, Length(VAR_FLAG)) then begin
          ...
          if GetMissionFalgStatus(n14) = 0 then
            sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', sWizard)
          else
            sMsg := AnsiReplaceText(sMsg, '<' + sLabel + '>', sTaos);
        end else
...
begin
  case Mag.Magic.wMagicId of
    ...
    100: Result := g_WMainImages.Images[909];
    110: Result := g_WMainImages.Images[952];
    111: Result := g_WMainImages.Images[950];
    112: Result := g_WMainImages.Images[956];
    113: Result := g_WMainImages.Images[954];
    114: Result := g_WMainImages.Images[944];
    115: Result := g_WMainImages.Images[942];
    116: Result := g_WMainImages.Images[946];
    117: Result := g_WMainImages.Images[940];
    118: Result := g_WMainImages.Images[934];
    119: Result := g_WMainImages.Images[936];
    120: Result := g_WMainImages.Images[932];
    121: Result := g_WMainImages.Images[930];
