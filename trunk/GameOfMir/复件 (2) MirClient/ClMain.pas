...
uses
  ...
  MShare, Share, jpeg, HGECanvas, RSA;
...
uses
  ...
  MD5Unit, Registry, FWeb, MNShare, CheckDLL, MyCommon, UrlMon, Logo, EncryptFile, FrmAD, ShellAPI, DlgConfig;
...
{$IF Var_Free = 1}
  CLIENTUPDATETIME := CLIENTUPDATETIME + '(免费版)';
{$IFEND}
...
{$IFEND}
  ...
  Try
    ...
    Try
      ...
      if Reg.OpenKey(REG_SETUP_PATH, True) then begin
        if ReadBool(Reg, REG_SETUP_BITDEPTH, False) then HGE.System_SetState(HGE_SCREENBPP, 32)
        else HGE.System_SetState(HGE_SCREENBPP, 16);
  ...
  ClientWidth := 800;
  ClientHeight := 600;
  //MyDevice.Width := SCREENWIDTH;
  //MyDevice.Height := SCREENHEIGHT;
  ...
  //Label1.Left := (SCREENWIDTH - label1.Width) div 2;
  //Label1.Top :=  (SCREENHEIGHT - label1.Height) div 2;
...
begin
  ...
  {if HGE.System_GetState(HGE_WINDOWED) <> (not boFull) then begin
    ...
    if (HGE.System_GetState(HGE_WINDOWED)) then begin
      ...
      ClientHeight := HGE.System_GetState(HGE_SCREENHEIGHT);
      BorderIcons := [biSystemMenu, biMinimize];
      Left := (Screen.width - SCREENWIDTH) div 2;
      Top := (Screen.Height - SCREENHEIGHT) div 2 - 40;
    end
    else begin
      BorderStyle := bsNone;
      BorderIcons := [];
    ...
    msgDeviceRestoreSize: begin
        ...
        if g_boFullScreen then begin
          if FIDDraw <> nil then begin
            if ClientWidth = 800 then FIDDraw.SetDisplayMode(800, 600, 16)
            else FIDDraw.SetDisplayMode(1024, 768, 16);
          end;
          m_Point.X := 0;
          m_Point.Y := 0;
        end else begin
...
{$IF Public_Ver = Public_Test}
    ...
    g_boSQLReg := False;
    g_ServerInfoCount := 4;
    g_ServerInfo[0].sName := '127.0.0.1:7000';
    g_ServerInfo[0].sAddrs := '127.0.0.1';
    g_ServerInfo[0].sPort := '7000';
    g_ServerInfo[1].sName := '192.168.1.98:7000';
    g_ServerInfo[1].sAddrs := '192.168.1.98';
    g_ServerInfo[1].sPort := '7000';
    g_ServerInfo[2].sName := '192.168.100.98:7000';
    g_ServerInfo[2].sAddrs := '192.168.100.98';
    g_ServerInfo[2].sPort := '7000';
    g_ServerInfo[3].sName := '192.168.1.100:7000';
    g_ServerInfo[3].sAddrs := '192.168.1.100';
...
{$ELSE}
    g_ServerInfoCount := 1;
    g_ServerInfo[0].sName := '测试服务器';
    if g_sMainParam1 = '' then begin
      {g_ServerInfo[0].sAddrs := '127.0.0.1,127.0.0.1,127.0.0.1';
      g_ServerInfo[0].sPort := '8888,7000,7001';    }
      g_ServerInfo[0].sAddrs := '127.0.0.1';
      g_ServerInfo[0].sPort := '7000';
      //g_ServerInfo[0].sAddrs := 's1.mir2k.com';
      //g_ServerInfo[0].sPort := '7001';
      //g_ServerInfo[0].sAddrs := 's1.mir2k.com,s1.mir2k.com,s1.mir2k.com';
      //g_ServerInfo[0].sPort := '7000,7001,7002';
    end else begin
      g_ServerInfo[0].sAddrs := g_sMainParam1;
      g_ServerInfo[0].sPort := g_sMainParam2;
    end;
{$ENDIF}
  ...
  begin
    if g_LogoSurface <> nil then begin
      if FnShowLogoIndex < 256 then begin
        g_DXCanvas.Draw((SCREENWIDTH - g_LogoSurface.Width) div 2,
          (SCREENHEIGHT - g_LogoSurface.Height) div 2 - 20,
...
begin
  ...
  Try
    ...
    BitmapBoldTextOut(Bitmap.Canvas, 2, 2, clWhite, clBlack, '361M2');
    if g_MySelf <> nil then begin
...
{$IF Var_Interface =  Var_Default}
      VK_ESCAPE: begin
        FrmDlg2.dwndSysSetup.Left := (SCREENWIDTH - FrmDlg2.dwndSysSetup.Width) div 2;
        FrmDlg2.dwndSysSetup.Top := (SCREENHEIGHT - FrmDlg2.dwndSysSetup.Height) div 2;
...
begin
  ...
  if (nX >= 0) and (nY >= 0) and (nX <= SCREENWIDTH) and (nY <= SCREENHEIGHT) then begin
...
{$IF Var_Interface = Var_Mir2}
      tdir := GetFlyDirection(364 + ((g_FScreenWidth - 800) div 2), 224 + ((g_FScreenHeight - 600) div 2 + 12), tx, ty);
{$ELSE}
      tdir := GetFlyDirection(364 + ((g_FScreenWidth - 800) div 2), 288 + ((g_FScreenHeight - 600) div 2 + 12), tx, ty);  
{$IFEND}
...
{$IFDEF DEBUG}
  ClientVersion := 506;
{$ELSE}
  ClientVersion := CLIENT_VERSION_NUMBER;
{$ENDIF}

  ClientVersion := ClientVersion and CLIENT_VERSION_MARK;
{$IFDEF DEBUG}
  ClientVersion := ClientVersion or CLIENT_VERSIONEX_DEBUG;
{$ENDIF}
{$IF Public_Ver = Public_Test}
  ClientVersion := ClientVersion or CLIENT_VERSIONEX_TEST;
{$IFEND}
{$IF Var_Free = 1}
  ClientVersion := ClientVersion or CLIENT_VERSIONEX_FREE;
{$IFEND}
{$IF Var_Interface = Var_Mir2}
  ClientVersion := ClientVersion or CLIENT_VERSIONEX_MIR2;
{$IFEND}

{$IFDEF DEBUG}
    sSendMsg := format('**%s/%s/%d/%d/%s/%d', [LoginID, CharName, Certification, ClientVersion, EncryStrHex(CharName, '19850506'),0]);
{$ELSE}
  {$IF (Public_Ver = Public_Test) or (Var_Free = 1)}
...
begin
  ...
  if nLen = GetCodeMsgSize((SizeOf(TMakeItemUserItem) - SizeOf(TUserItemValue)) * 4 / 3) then begin
...
begin
  if boReset then begin
    ...
    if not FboDisplayChange then begin
      ...
      if HGE.System_GetState(HGE_SCREENWIDTH) = 800 then begin
        nWidth := 800;
        nHeight := 600;
      end
      else begin
