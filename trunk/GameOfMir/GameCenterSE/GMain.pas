...
var
  ...
  I: Integer;
begin
  sIniFile := g_sGameDirectory + SERVERCONFIGDIR;
  ...
  IniGameConf.WriteString(DBSERVERSECTIONNAME2, 'DBDir', g_sGameDirectory + DBSERVERDBDIR);
  IniGameConf.Free;

  SaveList := TStringList.Create;
  SaveList.Add(sLocalIPaddr);
  SaveList.SaveToFile(sIniFile + DBSERVERALLOWADDR);
  ...
  SaveList.SaveToFile(g_sGameDirectory + SERVERCONFIGDIR + DBSERVERGATEINFO);
  SaveList.Free;

  sIniFile := g_sGameDirectory + SERVERGAMEDATADIR;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;

  sIniFile := g_sGameDirectory + DBSERVERDBDIR;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
end;
...
begin
  ...
  IniGameConf := Tinifile.Create(sIniFile + SERVERCONFIGFILE);
  IniGameConf.WriteString(LOGINSRVSECTIONNAME2, 'ServerAddr', sLocalIPaddr);
  ...
  IniGameConf.WriteString(LOGINSRVSECTIONNAME2, 'CountLogDir', g_sGameDirectory + LOGINSRVCHRLOGNAME);
  IniGameConf.Free;

  SaveList := TStringList.Create;
  SaveList.Add(sLocalIPaddr);
  SaveList.SaveToFile(sIniFile + LOGINSRVALLOWADDR);

  SaveList.Clear;
  SaveList.Add(format('%s %s %d', [g_sGameName, g_sGameName, nLimitOnlineUser]));
  SaveList.SaveToFile(sIniFile + LOGINSRVUSERLIMIT);
  ...
  SaveList.Add(sIniFile);
  SaveList.SaveToFile(g_sGameDirectory + SERVERCONFIGDIR + LOGINSRVGETINFO);


  sIniFile := g_sGameDirectory + SERVERLOGDIR;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;

  sIniFile := g_sGameDirectory + LOGINSRVCHRLOGNAME;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
end;
...
begin
  ...
  IniGameConf.WriteString(LOGSERVERSECTIONNAME2, 'BaseDir', g_sGameDirectory + LOGSERVERBaseDir);

  sIniFile := g_sGameDirectory + LOGSERVERBaseDir;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
  IniGameConf.Free;
...
begin
  ...
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'GuildDir', g_sGameDirectory + M2SERVERSEGuildDir);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'GuildFile', g_sGameDirectory + M2SERVERSEGuildFile);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'ConLogDir', g_sGameDirectory + M2SERVERSEConLogDir);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'CastleDir', g_sGameDirectory + M2SERVERSECastleDir);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'CastleFile', g_sGameDirectory + M2SERVERSECastleFile);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'GameDataDir', g_sGameDirectory + SERVERGAMEDATADIR);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'EnvirDir', g_sGameDirectory + M2SERVERSEnvirDir);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'MapDir', g_sGameDirectory + M2SERVERSMapDir);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'LogDir', g_sGameDirectory + M2SERVERSELogDir);
  IniGameConf.WriteString(M2SERVERSECTIONNAME2, 'EMailDir', g_sGameDirectory + M2SERVERSEmailDir);
  IniGameConf.Free;
  ...
  sIniFile := g_sGameDirectory + M2SERVERSEGuildBase;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
  sIniFile := g_sGameDirectory + M2SERVERSEGuildDir;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
  sIniFile := g_sGameDirectory + M2SERVERSECastleDir;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
  sIniFile := g_sGameDirectory + M2SERVERSEConLogDir;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
  sIniFile := g_sGameDirectory + M2SERVERSELogDir;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
  sIniFile := g_sGameDirectory + M2SERVERSEnvirDir;
  if not DirectoryExists(sIniFile) then begin
    CreateDir(sIniFile);
  end;
  ...
  SaveList := TStringList.Create;
  SaveList.Add(sLocalIPaddr);
  SaveList.SaveToFile(sIniFile + M2SERVERSALLOWADDR);
  SaveList.Free;
end;
...
begin
  if DBServer.boGetStart then begin
    case DBServer.btStartStatus of
      0: begin
          GetFileData(EXEDIRNAME + DBServer.sProgramFile, DataBuffer, nBufferLen);
          nRetCode := RunProgram(DBServer, IntToStr(Self.Handle), 0, DataBuffer, nBufferLen);
  ...
  if LoginServer.boGetStart then begin
    case LoginServer.btStartStatus of //
      0: begin
          GetFileData(EXEDIRNAME + LoginServer.sProgramFile, DataBuffer, nBufferLen);
          nRetCode := RunProgram(LoginServer, IntToStr(Self.Handle), 0, DataBuffer, nBufferLen);
          if nRetCode = 0 then begin
  ...
  if LogServer.boGetStart then begin
    case LogServer.btStartStatus of //
      0: begin
          GetFileData(EXEDIRNAME + LogServer.sProgramFile, DataBuffer, nBufferLen);
          nRetCode := RunProgram(LogServer, IntToStr(Self.Handle), 0, DataBuffer, nBufferLen);
          if nRetCode = 0 then begin
  ...
  if M2Server.boGetStart then begin
    case M2Server.btStartStatus of //
      0: begin
          GetFileData(EXEDIRNAME + M2Server.sProgramFile, DataBuffer, nBufferLen);
          nRetCode := RunProgram(M2Server, IntToStr(Self.Handle), 0, DataBuffer, nBufferLen);
          if nRetCode = 0 then begin
  ...
  for I := Low(RunGate) to High(RunGate) do begin
    if RunGate[I].boGetStart then begin
      if RunGate[I].btStartStatus = 0 then begin
        ...
        GetFileData(EXEDIRNAME + RunGate[I].sProgramFile, DataBuffer, nBufferLen);
        nRetCode := RunProgram(RunGate[I], IntToStr(Self.Handle), 0, DataBuffer, nBufferLen);
        if nRetCode = 0 then begin
  ...
  if SelGate.boGetStart then begin
    case SelGate.btStartStatus of //
      0: begin
          ...
          GetFileData(EXEDIRNAME + SelGate.sProgramFile, DataBuffer, nBufferLen);
          nRetCode := RunProgram(SelGate, IntToStr(Self.Handle), 0, DataBuffer, nBufferLen);
          if nRetCode = 0 then begin
  ...
  if SelGate1.boGetStart then begin
    case SelGate1.btStartStatus of //
      0: begin
          ...
          GetFileData(EXEDIRNAME + SelGate1.sProgramFile, DataBuffer, nBufferLen);
          nRetCode := RunProgram(SelGate1, IntToStr(Self.Handle), 0, DataBuffer, nBufferLen);
          if nRetCode = 0 then begin
  ...
  if LoginGate.boGetStart then begin
    case LoginGate.btStartStatus of //
      0: begin
          GetFileData(EXEDIRNAME + LoginGate.sProgramFile, DataBuffer, nBufferLen);
          nRetCode := RunProgram(LoginGate, IntToStr(Self.Handle), 0, DataBuffer, nBufferLen);
          if nRetCode = 0 then begin
...
begin
  g_Config.SelGate.GetStart2 := CheckBoxSelGate.Checked;
end;
...
begin
  if not m_boOpen then
    Exit;
  g_Config.SelGate.GetStart2 := CheckBoxboSelGate_GetStart.Checked;
end;
...
begin
  ...
  EditRunGate_GatePort1.Text := '9200';
  EditRunGate_GatePort2.Text := '9201';
  EditRunGate_GatePort3.Text := '9202';
  EditRunGate_GatePort4.Text := '9203';
  EditRunGate_GatePort5.Text := '9204';
  EditRunGate_GatePort6.Text := '9205';
  EditRunGate_GatePort7.Text := '9206';
  EditRunGate_GatePort8.Text := '9207';
...
begin
  EditLoginGate_MainFormX.Text := '0';
  EditLoginGate_MainFormY.Text := '0';
  EditLoginGate_GatePort.Text := '8888';
...
begin
  ...
  EditSelGate_GatePort.Text := '9100';
  EditSelGate_GatePort1.Text := '9101';
