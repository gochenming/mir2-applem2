...
const
  ...
  DBSERVERDBDIR = SERVERGAMEDATADIR + 'DB\';
  DBSERVERALLOWADDR = 'DBServer_AllowAddr.txt';
  DBSERVERGATEINFO = 'DBServer_GateInfo.txt';
...
function RunProgram(var ProgramInfo: TProgram; sHandle: string; dwWaitTime: LongWord; const ABuffer: PChar; nLen: Integer):
  LongWord;
...
var
  ...
  g_sConfFile: string = '.\Config\CenterConfig.ini';
  g_sBackListFile: string = '.\Config\CenterBackList.txt';

  g_sGameName: string = '新热血传奇';
  ...
  g_Config: TConfig = (
      ...
      LogServer:(
        ...
        Port: 10000;
        GetStart: True;
        ProgramFile: 'LogDataServer.dat';
      );
      RunGate:(
        MainFormX: 437;
        MainFormY: 373;
        GetStart: (True, True, True, False, False, False, False, False);
        GatePort: (9200, 9201, 9202, 9203, 9204, 9205, 9206, 9207);
        ProgramFile: 'RunGate.dat';
      );
      SelGate:(
        MainFormX: 0;
        MainFormY: 180;
...
begin
  Result := 0;
  sCommandLine := format('.\Execute\%s %s %d %d 1', [ProgramInfo.sProgramFile, sHandle, ProgramInfo.nMainFormX,
    ProgramInfo.nMainFormY]);
