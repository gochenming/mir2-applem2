...
interface
uses
  Windows, Classes, Graphics, SysUtils, TDX9Textures, HUtil32, WIL;

const
  HEADERNAME = 'PACK';
  COPYRIGHTNAME = ' v3.0 - 361m2.com';
  MYFILEEXT = '.pak';
...
begin
  ...
  if FHeader.ImageCount <= 0 then exit;
...
begin
  ...
  if index < FIndexList.Count then begin
    nPosition := Integer(FIndexList[index]);
    if FFileStream.Seek(nPosition, 0) <> nPosition then exit;
    if FFileStream.Read(DXInfo, SizeOf(TDXTextureInfo)) = SizeOf(TDXTextureInfo) then begin
      nX := DXInfo.px;
      nY := DXInfo.py;
    end;
    {FFileStream.Seek(-SizeOf(TDXTextureInfo), soFromCurrent);
