...
{$IF Var_Interface = Var_Mir2}
  MERCHANTMAXHEIGHT = 140;
  MERCHANTMAXWIDTH = 354;
  MDLGMAXWIDTH = 354;
  MDLGMAXHEIGHT = 1024;
{$ELSE}
...
begin
  ...
  if Sender = DBottom2 then begin
    ...
    with DUserKeyGrid1 do begin
      ...
      if (x >= 208) and (y >= 230) and (X <= 208 + 364) and (y <= 230 + 16) then begin
...
{$IF Var_Interface = Var_Mir2}
    ...
    d := g_WMainImages.Images[31];
    if d <> nil then
      dsurface.Draw(ax + 283, ay + 79, d.ClientRect, d, True);
    d := g_WMainImages.Images[32];
    if d <> nil then
      dsurface.Draw(ax + 326, ay + 79, d.ClientRect, d, True);
    d := g_WMainImages.Images[33];
    if d <> nil then
      dsurface.Draw(ax + 370, ay + 79, d.ClientRect, d, True);
    d := g_WMainImages.Images[34];
    if d <> nil then
      dsurface.Draw(ax + 414, ay + 79, d.ClientRect, d, True);
    d := g_WMainImages.Images[35];
    if d <> nil then
      dsurface.Draw(ax + 457, ay + 79, d.ClientRect, d, True);
    d := g_WMainImages.Images[36];
    if d <> nil then
      dsurface.Draw(ax + 502, ay + 79, d.ClientRect, d, True);

    d := nil;
    case HourOf(g_ServerDateTime) of
      0..6: d := g_WMainImages.Images[15]; //早上
      7..11: d := g_WMainImages.Images[12]; //白天
      12..18: d := g_WMainImages.Images[13]; //傍晚
      19..23: d := g_WMainImages.Images[14]; //晚上
    end;
    if d <> nil then
      dsurface.Draw(SCREENWIDTH div 2 + (SCREENWIDTH div 2 - (400 - 348)), ay + 79, d.ClientRect, d, FALSE);
    ...
    case g_nAreaStateValue of
      OT_SAFEAREA: d := g_WMainImages.Images[151];
      OT_SAFEPK: d := g_WMainImages.Images[150];
      OT_FREEPKAREA: d := g_WMainImages.Images[150];
    end;
    if d <> nil then
    ...
    with g_DXCanvas do begin
      if DBTAttackMode.Tag in [Low(AttackModeName)..High(AttackModeName)] then
        TextOut(ax + 648, ay + 113, clWhite, AttackModeName[DBTAttackMode.Tag]);

      TextOut(ax + 672, ay + 230, clWhite, FormatDateTime('HH:MM:SS', g_ServerDateTime));
      ...
      TextOut(ax + 679 - TextWidth(sStr) div 2, ay + 146, clWhite, sStr);
      TextOut(8, SCREENHEIGHT - 16, clWhite, g_sMapTitle + ' ' + IntToStr(g_MySelf.m_nCurrX) + ':' + IntToStr(g_MySelf.m_nCurrY));
    end;
{$IFEND}
  end;
end;
...
{$IF Var_Interface =  Var_Default}
  with Sender as TDWindow do begin
    ...
    d := WLib.Images[28];
    if d <> nil then
      DSurface.Draw(ax + 18, ay + 10, d.ClientRect, d, True);
...
{$IF Var_Interface = Var_Mir2}
  ...
  Dec(X, DBottom.Left);
  ShowMsg := '';
  if (x >= 640) and (x <= 640 + 105) and (y >= 172) and (y <= 172 + 26) then begin
    x := 640;
    Y := 192;
    ShowMsg := '经验值：' + IntToStr(g_MySelf.m_Abil.Exp) + '/' + IntToStr(g_MySelf.m_Abil.MaxExp);
  end else
  if (x >= 640) and (x <= 640 + 105) and (y >= 205) and (y <= 205 + 26) then begin
    x := 640;
    Y := 228;
  ...
  with Sender as TDGrid do begin
    ...
    if (Idx >= 0) and (idx < MagicList.Count) then begin
      ...
      if Magic.Magic.sMagicName <> '' then begin
        ...
        if g_MyMagicArry[nMagID].Level in [Low(Magic.Magic.MaxTrain)..High(Magic.Magic.MaxTrain)] then nMaxTrain := Magic.Magic.MaxTrain[g_MyMagicArry[nMagID].Level]
        else nMaxTrain := Magic.Magic.MaxTrain[High(Magic.Magic.MaxTrain)];
        ...
        if g_MyMagicArry[nMagID].btKey > 0 then begin
          d := g_WMain3Images.Images[147 + g_MyMagicArry[nMagID].btKey];
          if d <> nil then
...
begin
  with Sender as TDGrid do begin
    if NpcGoodsIdx = 0 then begin
      ...
      if idx in [Low(NpcGoodItems)..High(NpcGoodItems)] then begin
...
{$IF Var_Interface = Var_Mir2}
      d := g_WMainImages.Images[29];
      if d <> nil then
        dsurface.Draw(ax + 9, ay + 190, d.ClientRect, d, False);
{$IFEND}
      with g_DXCanvas do begin
{$IF Var_Interface = Var_Mir2}
        TextOut(ax + 73, ay + 193, GetGoldStr(g_MySelf.m_nGold), clWhite);
        TextOut(ax + 134, ay + 217, GetGoldStr(g_nBindGold), clWhite);
        TextOut(ax + 134, ay + 238, GetGoldStr(g_MySelf.m_nGameGold), $CCFF);
{$ELSE}
        TextOut(ax + Width div 2 - TextWidth(Caption) div 2, ay + 12, $B1D2B7, Caption);
...
begin
  ...
  if idx in [Low(g_ItemArr)..High(g_ItemArr)] then begin
    if (not g_boItemMoving) and (g_ItemArr[idx].S.name <> '') then begin
      ...
      if FrmDlg3.DWndArmStrengthen.Visible and FrmDlg3.CanArmStrengthenAdd(@g_ItemArr[idx]) then begin
      ...
      if g_ItemArr[idx].S.name <> '' then begin
        ...
        if d <> nil then begin
          ...
          if (SysTick < FShowItemEffectTick) and (g_ItemArr[idx].UserItem.MakeIndex = FShowItemEffectIndex) then begin
            d := g_WMain2Images.Images[230 + (FShowItemEffectTick - SysTick) div 150];
            if d <> nil then begin
...
{$IF Var_Interface = Var_Mir2}
  ...
  if ((X - DMerchantDlg.Left - 20) < MERCHANTMAXWIDTH) and ((Y - DMerchantDlg.Top - 18) < MERCHANTMAXHEIGHT) then begin
{$ELSE}
  L := DMerchantDlg.Left + 20;
  T := DMerchantDlg.Top + 20;
  if ((X - DMerchantDlg.Left - 20) < MERCHANTMAXWIDTH) and ((Y - DMerchantDlg.Top - 20) < MERCHANTMAXHEIGHT) then begin
{$IFEND}
...
begin
  with Sender as TDWindow do begin
    ...
    if d <> nil then begin
      ...
      rc.Bottom := DMDlgUpDonw.Position + MerchantMaxHeight;
{$IF Var_Interface = Var_Mir2}
      Inc(ax, 20);
      Inc(ay, 18);
{$ELSE}
...
begin
  ...
  if d <> nil then begin
    ...
    ModalWindow.Left := (SCREENWIDTH - d.Width) div 2;
    ModalWindow.Top := (SCREENHEIGHT - d.Height) div 2;
  end;
  DEdit := nil;
  DOKButton := nil;
  DNoButton := nil;
  ...
  try
    ...
    ModalWindow.Left := (SCREENWIDTH - nWidth) div 2;
    ModalWindow.Top := (SCREENHEIGHT - nHeight) div 2;
...
{$IF Var_Interface = Var_Mir2}
    d := g_WUI1Images.Images[2511];
    if d <> nil then begin
      DlgInfo[0].WMImages := g_WUI1Images;
      DlgInfo[0].Index := 2511;
      DlgInfo[0].Rect.Left := 0;
      DlgInfo[0].Rect.Top := 0;
      DlgInfo[0].Rect.Bottom := d.Height;
      DlgInfo[0].Rect.Right := Width;
    end;
    d := g_WUI1Images.Images[2531];
    if d <> nil then begin
      DlgInfo[1].WMImages := g_WUI1Images;
      DlgInfo[1].Index := 2531;
      DlgInfo[1].Rect.Left := 0;
      DlgInfo[1].Rect.Top := Height - d.Height;
      DlgInfo[1].Rect.Bottom := Height;
      DlgInfo[1].Rect.Right := Width;
    end;
    d := g_WUI1Images.Images[2520];
    if d <> nil then begin
      DlgInfo[2].WMImages := g_WUI1Images;
      DlgInfo[2].Index := 2520;
      DlgInfo[2].Rect.Left := 0;
      DlgInfo[2].Rect.Top := 0;
      DlgInfo[2].Rect.Bottom := Height;
      DlgInfo[2].Rect.Right := d.Width;
    end;
    d := g_WUI1Images.Images[2522];
    if d <> nil then begin
      DlgInfo[3].WMImages := g_WUI1Images;
      DlgInfo[3].Index := 2522;
    ...
    d := g_WUI1Images.Images[2510];
    if d <> nil then begin
      DlgInfo[4].WMImages := g_WUI1Images;
      DlgInfo[4].Index := 2510;
      DlgInfo[4].Rect.Left := 0;
      DlgInfo[4].Rect.Top := 0;
    end;
    d := g_WUI1Images.Images[2532];
    if d <> nil then begin
      DlgInfo[5].WMImages := g_WUI1Images;
      DlgInfo[5].Index := 2532;
      DlgInfo[5].Rect.Left := Width - d.Width;
      DlgInfo[5].Rect.Top := 0;
    end;
    d := g_WUI1Images.Images[2530];
    if d <> nil then begin
      DlgInfo[6].WMImages := g_WUI1Images;
      DlgInfo[6].Index := 2530;
      DlgInfo[6].Rect.Left := 0;
      DlgInfo[6].Rect.Top := Height - d.Height;
    end;
    d := g_WUI1Images.Images[2532];
    if d <> nil then begin
      DlgInfo[7].WMImages := g_WUI1Images;
      DlgInfo[7].Index := 2532;
...
begin
  if not DBTSayMove.Downed then
    exit;
  nCount := (SCREENHEIGHT - Y - DBottom.Height - 39 + 13) div SAYLISTHEIGHT;
  if nCount < 3 then
    nCount := 3;
  if nCount > 29 then
    nCount := 29;
  ...
  DWndSay.Top := SCREENHEIGHT - DWndSay.Height - DBottom.Height - 39;
...
{$IFEND}
    ...
    with g_DXCanvas do begin
{$IF Var_Interface = Var_Mir2}
      TextOut(ax + Width div 2 - TextWidth(g_MySelf.m_UserName) div 2, ay + 17, $B1D2B7, g_MySelf.m_UserName);
{$ELSE}
      TextOut(ax + Width div 2 - TextWidth(g_MySelf.m_UserName) div 2, ay + 12, $B1D2B7, g_MySelf.m_UserName);
{$IFEND}
...
{$IF Var_Interface = Var_Mir2}
  with Sender as TDButton do begin
    if g_MyMagicArry[SKILL_CBO].btKey = 0 then begin
      ...
      d := g_WMain3Images.Images[147 + g_MyMagicArry[SKILL_CBO].btKey];
      if d <> nil then begin
    ...
    if WLib <> nil then begin
      if Downed then begin
        ...
        if idx in [0..3] then begin
          d := g_WMain3Images.Images[307 + idx];
          if d <> nil then begin
    ...
    if WLib <> nil then begin
      ...
      if idx = 0 then begin
        ...
        if idx in [0..11] then begin
          d := g_WMainImages.Images[700 + idx];
              ...
              if g_MyMagicArry[nMagID].boNotUse then begin
                ...
                if g_MyMagicArry[nMagID].boUse and not boUse and not g_boLatestSpell then begin
                  if (g_MyMagicArry[nMagID].btEffIdx < 9) then begin
                    ...
                    d := g_WMainImages.GetCachedImage(FLASHBASE + g_MyMagicArry[nMagID].btEffIdx, ax, ay);
                    if d <> nil then begin
...
{$IFEND}
    ...
    DWinSelServer.Left := (SCREENWIDTH - d.Width) div 2;
    DWinSelServer.Top := (SCREENHEIGHT - d.Height) div 2;
  end;
{$IF Var_Interface = Var_Mir2}
  dbtnSelServerClose.SetImgIndex(g_WMainImages, 64);
...
{$IF Var_Interface = Var_Mir2}
  ...
  if d <> nil then begin
    ...
    DWndHint.Left := (SCREENWIDTH - d.Width) div 2;
    DWndHint.Top := (SCREENHEIGHT - d.Height) div 2;
  end;
...
{$IF Var_Interface = Var_Mir2}
  ...
  DLoginExit.SetImgIndex(g_WMainImages, 64);
  DLoginExit.Left := 252;
  DLoginExit.Top := 28;
...
{$ELSE}
  ...
  if d <> nil then begin
    ...
    DCreateChr2.Left := (SCREENWIDTH - d.Width) div 2;
    DCreateChr2.Top := (SCREENHEIGHT - d.Height) div 2;
  end;
...
{$IF Var_Interface = Var_Mir2}
  ...
  if d <> nil then begin
    DBottom.SetImgIndex(g_WMain3Images, 291);
    DBottom.Left := 0;
  ...
  DBotMusic.SetImgIndex(g_WMainImages, 11);
  DBotMusic.Left := 764;
  DBotMusic.Top := 11;
  ...
  DBotAddAbil.SetImgIndex(g_WMainImages, 140);
  DBotAddAbil.Left := 209 + 30 * 9;
  DBotAddAbil.Top := 104;
  DBotAddAbil.Visible := True;


  DTopShop.SetImgIndex(g_WMain3Images, 297);
  DTopShop.Left := 754;
  DTopShop.Top := 204;
  ...
  DTopHelp.SetImgIndex(g_WMain2Images, 495);
  DTopHelp.Left := 601;
  DTopHelp.Top := 59;
  ...
  DTopEMail.SetImgIndex(g_WMain2Images, 498);
  DTopEMail.Left := 170;
  DTopEMail.Top := 60;
  ...
  DSayUpDown.SetImgIndex(g_WUI3Images, 500);
  DSayUpDown.Top := 118;
  DSayUpDown.Left := 597;
  DSayUpDown.Height := 112;

  DSayUpDown.UpButton.SetImgIndex(g_WUI3Images, 501);
  DSayUpDown.DownButton.SetImgIndex(g_WUI3Images, 504);
  DSayUpDown.MoveButton.SetImgIndex(g_WUI3Images, 507);
...
{$IF Var_Interface = Var_Mir2}
  ...
  DItemBagShop.SetImgIndex(g_WMain3Images, 245);
  DItemBagShop.Top := 178;
  DItemBagShop.Left := 212;
  DItemBagShop.OnDirectPaint := DBTHintCloseDirectPaint;
  DItemBagShop.DFColor := $ADD6EF;
  DItemBagShop.Caption := '摆摊界面';
  DItemBagRef.SetImgIndex(g_WMain99Images, 330);
  DItemBagRef.Top := 478 + 21;
  DItemBagRef.Left := 115;
  ...
  DCloseBag.SetImgIndex(g_WMainImages, 64);
  DCloseBag.Left := 328;
  DCloseBag.Top := 30;
{$ELSE}
  DItemBag.SetImgIndex(g_WMain99Images, 132);
  DItemBag.Left := SCREENWIDTH - DItemBag.Width;
  DItemBag.Top := 0;

  DItemBagShop.SetImgIndex(g_WMain99Images, 156);
...
{$IF Var_Interface = Var_Mir2}
  ...
  DWndFace.Left := SCREENWIDTH - DWndFace.Width - 205;
...
{$IF Var_Interface = Var_Mir2}
  ...
  DCloseState.SetImgIndex(g_WMainImages, 64);
  DCloseState.Top := 34;
...
{$IF Var_Interface = Var_Mir2}
  ...
  DMagicFront.SetImgIndex(g_WUI1Images, 2206);
  DMagicFront.Top := 102;
  DMagicFront.Left := 238;
  DMagicNext.SetImgIndex(g_WUI1Images, 2209);
  DMagicNext.Top := 154;
  DMagicNext.Left := 238;
...
{$IF Var_Interface = Var_Mir2}
  ...
  DStateWinMagicCboClose.SetImgIndex(g_WMainImages, 64);
...
{$IF Var_Interface = Var_Mir2}
  ...
  DStateInfoSex.SetImgIndex(g_WUI1Images, 2750);
  ...
  DStateInfoSex.UpDown.UpButton.SetImgIndex(g_WUI1Images, 1613);
  DStateInfoSex.UpDown.DownButton.SetImgIndex(g_WUI1Images, 1616);
  DStateInfoSex.UpDown.MoveButton.SetImgIndex(g_WUI1Images, 1610);

  DStateInfoProvince.SetImgIndex(g_WUI1Images, 2750);
  ...
  DStateInfoProvince.UpDown.UpButton.SetImgIndex(g_WUI1Images, 1613);
  DStateInfoProvince.UpDown.DownButton.SetImgIndex(g_WUI1Images, 1616);
  DStateInfoProvince.UpDown.MoveButton.SetImgIndex(g_WUI1Images, 1610);

  DStateInfoCity.SetImgIndex(g_WUI1Images, 2750);
  ...
  DStateInfoCity.UpDown.UpButton.SetImgIndex(g_WUI1Images, 1613);
  DStateInfoCity.UpDown.DownButton.SetImgIndex(g_WUI1Images, 1616);
  DStateInfoCity.UpDown.MoveButton.SetImgIndex(g_WUI1Images, 1610);

  DStateInfoArea.SetImgIndex(g_WUI1Images, 2750);
  ...
  DStateInfoArea.UpDown.UpButton.SetImgIndex(g_WUI1Images, 1613);
  DStateInfoArea.UpDown.DownButton.SetImgIndex(g_WUI1Images, 1616);
  DStateInfoArea.UpDown.MoveButton.SetImgIndex(g_WUI1Images, 1610);
...
{$IF Var_Interface = Var_Mir2}
  ...
  DCloseUserState.SetImgIndex(g_WMainImages, 64);
...
{$IF Var_Interface = Var_Mir2}
  ...
  DMDlgUpDonw.UpButton.SetImgIndex(g_WUI1Images, 1613);
  DMDlgUpDonw.DownButton.SetImgIndex(g_WUI1Images, 1616);
  DMDlgUpDonw.MoveButton.SetImgIndex(g_WUI1Images, 1610); //114
{$ELSE}
...
{$IF Var_Interface = Var_Mir2}
  ...
  DMenuUpDonw.UpButton.SetImgIndex(g_WUI1Images, 1613);
  DMenuUpDonw.DownButton.SetImgIndex(g_WUI1Images, 1616);
  DMenuUpDonw.MoveButton.SetImgIndex(g_WUI1Images, 1610);
...
{$IF Var_Interface = Var_Mir2}
  ...
  DBuyAdd.SetImgIndex(g_WMainImages, 213);
  DBuyAdd.Left := 106;
  DBuyAdd.Top := 67;
  DBuyDel.SetImgIndex(g_WMainImages, 215);
  DBuyDel.Left := 122;
  DBuyDel.Top := 67;
...
{$IF Var_Interface = Var_Mir2}
  ...
  if d <> nil then begin
    ...
    DDealDlg.Left := (SCREENWIDTH - d.Width) div 2;
    DDealDlg.Top := 60;
  end;
...
{$IF Var_Interface = Var_Mir2}
  ...
  if d <> nil then begin
    ...
    DGroupDlg.Top := SCREENHEIGHT - d.Height - 199;
  end;
  ...
  DGroupUpDown.UpButton.SetImgIndex(g_WUI1Images, 1613);
  DGroupUpDown.DownButton.SetImgIndex(g_WUI1Images, 1616);
  DGroupUpDown.MoveButton.SetImgIndex(g_WUI1Images, 1610);
...
begin
  ...
  DMerchantDlg.Surface.Size := Point(MDLGMAXWIDTH, MDLGMAXHEIGHT);
  DMerchantDlg.Surface.PatternSize := Point(MDLGMAXWIDTH, MDLGMAXHEIGHT);
...
begin
  if g_QuestMsgList.Count > 0 then begin

    DBTCheck1.Left := 390 - (g_QuestMsgList.Count * 32 div 2);
...
begin
  DMenuUpDonw.MaxPosition := 0;
  if NpcGoodsList.Count > 0 then begin
    btIdx := 0;
    for I := 0 to NpcGoodsList.Count - 1 do begin
...
var
  ...
  idx: integer;
begin
  if NewClientItem.s.Effect > 0 then begin
    idx := (GetTickCount - g_dwDefTime) div 100 mod 20;
    dd := g_WItemEffectImages.Images[(NewClientItem.s.Effect - 1) * 20 + idx];
    if dd <> nil then
      d := dd;
  ...
  if (pmShowLevel in PaintMode) and (pRect <> nil) then begin
    if (NewClientItem.UserItem.EffectValue.btEffect in [1..9]) then begin
      if NewClientItem.UserItem.EffectValue.btEffect in [1..4] then begin
        case NewClientItem.UserItem.EffectValue.btEffect of
          1: begin
        ...
        idx := (GetTickCount - g_dwDefTime) div 150 mod 10;
        if NewClientItem.UserItem.EffectValue.btEffect = 9 then begin
          idx := 970 + idx;
        end
        else if NewClientItem.UserItem.EffectValue.btEffect = 8 then begin
          idx := 960 + idx;
        end
        else if NewClientItem.UserItem.EffectValue.btEffect = 7 then begin
          idx := 980 + idx;
        end
        else if NewClientItem.UserItem.EffectValue.btEffect = 6 then begin
          idx := 1000 + idx;
        end
        else if NewClientItem.UserItem.EffectValue.btEffect = 5 then begin
          idx := 990 + idx;
        end
        else
          idx := 0;
...
{$IFEND}
  ...
  if MerchantHeight > MerchantMaxHeight then begin
    DMDlgUpDonw.MaxPosition := MerchantHeight - MerchantMaxHeight;
