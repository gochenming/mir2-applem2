...
uses
  ...
  FindMapPath, WMFile, Dialogs, MudUtil, DirectXGraphics;
...
type
  ...
  TMoveItemState = set of (mis_MyDeal, mis_Sell, mis_Repair, mis_SuperRepair, mis_Storage, mis_Bag, mis_State,
    ...
    mis_buy, mis_EMailItem, mis_EMailReadItem, mis_bottom, mis_ArmStrengthenAdd, mis_MakeItemAdd,
    mis_ItemUnsealAdd, mis_ItemRemoveStoneAdd);
...
begin
  if nEffectIdx > 119399 then begin
    ...
    if nEffectIdx > 11999 then begin
      FileIdx := 1;
  ...
  Try
    ...
    case nImageType of
      1: begin
        ...
        if (Bmp.Width < 10) or (Bmp.Height < 10) or (Bmp.Width > SCREENWIDTH) or (Bmp.Height > SCREENHEIGHT)
        then begin
          Bmp.Free;
          Bmp := nil;
        end;
      ...
      2: begin
        ...
        if (Gif.Width > 10) or (Gif.Height > 10) or (Gif.Width <= SCREENWIDTH) or (Gif.Height <= SCREENHEIGHT)
        then begin
          Bmp := TBitmap.Create;
          Bmp.Assign(Gif);
        end;
      ...
      3: begin
        ...
        if (Jpeg.Width > 10) or (Jpeg.Height > 10) or (Jpeg.Width <= SCREENWIDTH) or (Jpeg.Height <= SCREENHEIGHT)
        then begin
          Bmp := TBitmap.Create;
          Bmp.Assign(Jpeg);
        end;
      ...
      tm_Weapon,
        ...
        tm_Light: begin
         ...
         { ShowString := ShowString + '攻击      45-535  [+15]\';
          ...
          TempInt := Item.s.Luck + Item.UserItem.Value.btValue[tb_Luck] - Item.UserItem.Value.btValue[tb_UnLuck];
          if TempInt > 0 then
...
{$ENDIF}
  ...
  if (Item.s.SetItemList <> nil) and (Item.s.SetItemList.Count > 0) then begin
    for I := 0 to Item.s.SetItemList.Count - 1 do begin
      ...
      if boOK then sNameColor := '$63CEFF'
        ...
        if CompareLStr(scmdparam, 'item', length('item')) then begin //new
          ...
          if Stditem.Name <> '' then begin
            ...
            if (nMin >= 0) and (nMin <= ITEMCOUNT) and (d <> nil) then begin
              ...
              if Stditem.Effect > 0 then begin
                New(ShowHint);
                g_TempList.Clear;
                for k := 0 to 19 do
                  g_TempList.Add(IntToStr((Stditem.Effect - 1) * 20 + k));
                SafeFillChar(ShowHint^, SizeOf(TNewShowHint), #0);
