...
begin
  ...
  m_MapSurface.Size := Point(MAPSURFACEWIDTH + UNITX * 8, MAPSURFACEHEIGHT + UNITY * 8);
  {m_MapSurface.Format := PresentParams.BackBufferFormat;
  m_MapSurface.MipMapping := False;
  m_MapSurface.Behavior := tbRTarget;  }
  m_MapSurface.Active := True;
  if not m_MapSurface.Active then
    exit;

  m_ObjSurface := TDXRenderTargetTexture.Create(g_DXCanvas);
  m_ObjSurface.Size := Point(MAPSURFACEWIDTH - SOFFX * 2, MAPSURFACEHEIGHT);
  {m_ObjSurface.Format := PresentParams.BackBufferFormat;
  m_ObjSurface.MipMapping := False;
  m_ObjSurface.Behavior := tbRTarget;  }
  m_ObjSurface.Active := True;
  if not m_ObjSurface.Active then
    exit;

  m_MagSurface := TDXRenderTargetTexture.Create(g_DXCanvas);
  m_MagSurface.Size := Point(MAPSURFACEWIDTH - SOFFX * 2, MAPSURFACEHEIGHT);
  {m_MagSurface.Format := PresentParams.BackBufferFormat;
  m_MagSurface.MipMapping := False;
  m_MagSurface.Behavior := tbRTarget;  }
  m_MagSurface.Active := True;
  if not m_MagSurface.Active then
    exit;

  {m_MissionSurface := TDXImageTexture.Create(FrmMain.MyDevice.Canvas);
  m_MissionSurface.Size := Point(MISSIONHINTWIDTH, MISSIONHINTHEIGHT);
  m_MissionSurface.PatternSize := Point(MISSIONHINTWIDTH, MISSIONHINTHEIGHT);
  m_MissionSurface.Format := D3DFMT_A4R4G4B4;
  m_MissionSurface.Active := True;
  if not m_MissionSurface.Active then
    exit;    }
{$IF Var_Interface =  Var_Default}
  m_OperateHintSurface := MakeDXImageTexture(OPERATEHINTWIDTH, OPERATEHINTHEIGHT, WILFMT_A4R4G4B4);
  if m_OperateHintSurface = nil then exit;
  m_OperateHintSurface.Canvas := g_DXCanvas;
{$IFEND}

  m_HealthBarSurface := MakeDXImageTexture(30, 6, WILFMT_R5G6B5);

  if m_HealthBarSurface = nil then exit;
  m_HealthBarSurface.Canvas := g_DXCanvas;
...
begin
  ...
  //  drawingbottomline := 0; //jacky
   { if (g_MySelf = nil) then begin
      ...
      g_CloseAllSys := False; //¾Ü¾øËùÓÐ¹«ÁÄÐÅÏ¢
      g_CloseCrcSys := False; //¾Ü¾øËùÓÐº°»°ÐÅÏ¢
      g_CloseMySys := False; //¾Ü¾øËùÓÐË½ÁÄÐÅÏ¢
      g_CloseGuildSys := False; //¾Ü¾øËùÓÐÐÐ»áÐÅÏ¢
      g_AutoSysMsg := False; //×Ô¶¯º°»°
      g_AutoMsg := ''; //×Ô¶¯º°»°ÄÚÈÝ
      if g_boCanDraw then begin
        with g_DXCanvas do begin
          //SetBkMode(Handle, TRANSPARENT);
          TextOut((SCREENWIDTH - TextWidth(msgstr)) div 2, (SCREENHEIGHT - 600) + 200, clWhite, msgstr);
          //Release;
        end;
      end;
      Exit;
  ...
  {try
    ...
    for k := 0 to EventMan.EventList.count - 1 do begin
      ...
      if (abs(evn.m_nX - g_MySelf.m_nCurrX) > 15) and (abs(evn.m_nY - g_MySelf.m_nCurrY) > 15) and
        (not evn.m_boClient) then begin
        evn.Free;
  ...
  try
    with Map.m_ClientRect do begin
{$IF SWH = SWH800}
      Left := g_MySelf.m_nRx - 10;
      Top := g_MySelf.m_nRy - 13;
      Right := g_MySelf.m_nRx + 10; // ¿À¸¥ÂÊ Â¥Åõ¸® ±×¸²
      Bottom := g_MySelf.m_nRy + 10;
...
{$IF Var_Interface = Var_Mir2}
  ...
  for I := g_CenterMsgList.Count - 1 downto 0 do begin
    ...
    g_DXCanvas.TextOut(400 - g_DXCanvas.TextWidth(sShowStr) div 2, nHeight - n * 18, CenterMsg.nFColor, sShowStr);
    Inc(n);
  end;
{$ELSE}
  nHeight := 450;
  for I := g_CenterMsgList.Count - 1 downto 0 do begin
    ...
    CenterMsg := g_CenterMsgList[I];
    sShowStr := Format(CenterMsg.sMsgStr, [_MAX(1, (CenterMsg.nTime - GetTickCount) div 1000)]);
    g_DXCanvas.TextOut(294 + ((506 - g_DXCanvas.TextWidth(sShowStr)) div 2), nHeight - n * 18, CenterMsg.nFColor, sShowStr);
    Inc(n);
  end;
{$IFEND}
          ...
          if m_MissionStatus <> NPCMS_None then begin
            ...
            if d <> nil then begin
              ...
              m_ObjSurface.Draw(Actor.m_nSayX - d.Width div 2,
                ...
                d.ClientRect, d, True);
              d := g_WMain2Images.Images[230 + m_dwMissionIconIdx];
              if d <> nil then begin
...
{$IF Var_Interface =  Var_Default}
  if m_OperateHintSurface <> nil then
    m_ObjSurface.Draw(OPERATEHINTX, OPERATEHINTY, m_OperateHintSurface.ClientRect, m_OperateHintSurface, True);
{$IFEND}
...
{$IF Var_Interface = Var_Mir2}
   m_ObjSurface.Draw(0, 0,
    ...
    UNITX * 4 + g_MySelf.m_nShiftX + MAPSURFACEWIDTH,
    UNITY * 5 + g_MySelf.m_nShiftY + MAPSURFACEHEIGHT),
    m_MapSurface,
    FALSE);
   defx := -UNITX * 3 - g_MySelf.m_nShiftX + AAX + 14;
   defy := -UNITY * 5 - g_MySelf.m_nShiftY;
{$ELSE}
   m_ObjSurface.Draw(0, 0,
    Rect(UNITX * 4 + g_MySelf.m_nShiftX,
    UNITY * 3 + g_MySelf.m_nShiftY,
    UNITX * 4 + g_MySelf.m_nShiftX + MAPSURFACEWIDTH,
    UNITY * 3 + g_MySelf.m_nShiftY + MAPSURFACEHEIGHT),
    m_MapSurface,
    FALSE);
  defx := -UNITX * 3 - g_MySelf.m_nShiftX + AAX + 14;
        ...
        for ii := m_DrawArray[k].ItemList.Count - 1 downto 0 do begin
          ...
          if DropItem <> nil then begin
            ...
            if (DropItem <> g_FocusItem) and
              ...
              if DropItem.BoFlash then begin
                DSurface := g_WMainImages.GetCachedImage(FLASHBASE + I, ax, ay);
...
{$IF SWH = SWH800}
  ...
  sy := (cy - g_MySelf.m_nRy) * UNITY + 224{192} + UNITY div 2 - g_MySelf.m_nShiftY;
{$ELSEIF SWH = SWH1024}
  sx := (cx - g_MySelf.m_nRx) * UNITX + 485 {364} + UNITX div 2 -
  g_MySelf.m_nShiftX;
  sy := (cy - g_MySelf.m_nRy) * UNITY + 270 {192} + UNITY div 2 -
  g_MySelf.m_nShiftY;
{$IFEND}
{$ELSE}
{$IF SWH = SWH800}
  sx := (cx - g_MySelf.m_nRx) * UNITX + 364 + UNITX div 2 - g_MySelf.m_nShiftX;
  sy := (cy - g_MySelf.m_nRy) * UNITY + {192} 288 + UNITY div 2 - g_MySelf.m_nShiftY;
{$ELSEIF SWH = SWH1024}
  sx := (cx - g_MySelf.m_nRx) * UNITX + 485 {364} + UNITX div 2 -
  g_MySelf.m_nShiftX;
  sy := (cy - g_MySelf.m_nRy) * UNITY + 270 {192} + UNITY div 2 -
...
{$IF Var_Interface = Var_Mir2}
{$IF SWH = SWH800}
  ccx := Round((mx - 364 + g_MySelf.m_nShiftX - UNITX div 2) / UNITX) + g_MySelf.m_nRx;
  ccy := Round((my - 224{192} + g_MySelf.m_nShiftY - UNITY div 2) / UNITY) + g_MySelf.m_nRy;
{$ELSEIF SWH = SWH1024}
...
{$IF SWH = SWH800}
  ccx := Round((mx - 364 + g_MySelf.m_nShiftX - UNITX div 2) / UNITX) + g_MySelf.m_nRx;
  ccy := Round((my - {192} 288 + g_MySelf.m_nShiftY - UNITY div 2) / UNITY) + g_MySelf.m_nRy;
{$ELSEIF SWH = SWH1024}
  ccx := Round((mx - 485 {364} + g_MySelf.m_nShiftX - UNITX) / UNITX) + g_MySelf.m_nRx;
  ccy := Round((my - 270 {192} + g_MySelf.m_nShiftY - UNITY) / UNITY) + g_MySelf.m_nRy;
{$IFEND}
    ...
    for ClientMission in MissionList do begin
      ...
      if (MissionInfo <> nil) and (not ClientMission.boAccept) then begin
        ...
        if (ClientMission.CompleteFlag[0].nFlag <> 0) and
          (GetMissionFalgStatus(ClientMission.CompleteFlag[0].nFlag) <> ClientMission.CompleteFlag[0].btValue) then
          MissionStatus := OldMissionStatus;
        if (MissionStatus = NPCMS_Complete) and (ClientMission.CompleteFlag[1].nFlag <> 0) and
          (GetMissionFalgStatus(ClientMission.CompleteFlag[1].nFlag) <> ClientMission.CompleteFlag[1].btValue) then
          MissionStatus := OldMissionStatus;
        if (MissionStatus = NPCMS_Complete) and (ClientMission.CompleteFlag[2].nFlag <> 0) and
          (GetMissionFalgStatus(ClientMission.CompleteFlag[2].nFlag) <> ClientMission.CompleteFlag[2].btValue) then
          MissionStatus := OldMissionStatus;
      ...
      if ClientMission.boAccept and (OldMissionStatus <> NPCMS_Complete) and (OldMissionStatus <> NPCMS_Accept) then begin
        ...
        if (ClientMission.AcceptFlag[0].nFlag <> 0) and
          (GetMissionFalgStatus(ClientMission.AcceptFlag[0].nFlag) <> ClientMission.AcceptFlag[0].btValue) then
          MissionStatus := OldMissionStatus;
        if (MissionStatus = NPCMS_Accept) and (ClientMission.AcceptFlag[1].nFlag <> 0) and
          (GetMissionFalgStatus(ClientMission.AcceptFlag[1].nFlag) <> ClientMission.AcceptFlag[1].btValue) then
          MissionStatus := OldMissionStatus;
        if (MissionStatus = NPCMS_Accept) and (ClientMission.AcceptFlag[2].nFlag <> 0) and
          (GetMissionFalgStatus(ClientMission.AcceptFlag[2].nFlag) <> ClientMission.AcceptFlag[2].btValue) then
          MissionStatus := OldMissionStatus;
