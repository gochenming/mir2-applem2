...
begin
  ...
  m_HintSurface.Size := Point(MAPSURFACEWIDTH, MAPSURFACEHEIGHT);
  m_HintSurface.PatternSize := Point(MAPSURFACEWIDTH, MAPSURFACEHEIGHT);
  ...
  with SaySurface do begin
    ...
    while True do begin
      ...
      if boCmd then begin
        ...
        if byte(str[i]) >= ADDSAYWHDTH then begin
          cmdstr := cmdstr + str[i];
          Inc(i);
          if i <= len then
            cmdstr := cmdstr + str[i]
        ...
        fdata := str[i];
      if (nLeng + g_DXCanvas.TextWidth(temp + fdata)) > (SCREENWIDTH - 10) then begin
        TextOutEx(nLeng, 2, temp, fColor, bColor);
...
{$IF Var_Interface = Var_Mir2}
  SayMessage := NewSayMsgEx(SAYLISTWIDTH, SAYLISTHEIGHT, UserSayType, BColor);
{$ELSE}
  SayMessage := NewSayMsg(SAYLISTWIDTH, SAYLISTHEIGHT, UserSayType);
{$IFEND}
    ...
    for I := 1 to Length(WideStr) do begin
      ...
      if boImage then begin
        if tstr = '#' then begin
          ...
          if nIndex in [Low(g_FaceIndexInfo)..High(g_FaceIndexInfo)] then begin
            if (nLen + SAYFACEWIDTH) > (SAYLISTWIDTH - 1) then begin
      ...
      if boClickName then begin
        if tstr = #7 then begin
          ...
          if (nLen + nTextLen) > (SAYLISTWIDTH - 1) then begin
            WideStr2 := cmdstr;
            cmdstr := '';
            AddStr2 := '';
            for ii := 1 to Length(WideStr2) do begin
              ...
              if (nLen + g_DXCanvas.TextWidth(AddStr2 + tstr2)) > (SAYLISTWIDTH - 1) then begin
...
{$IFEND}
        ...
        if (nLen + g_DXCanvas.TextWidth(AddStr + tstr)) > (SAYLISTWIDTH - 1) then begin
          TextOutEx(nLen, 2, AddStr, nFColor, nBColor);
          Inc(nLen, g_DXCanvas.TextWidth(AddStr));
          AddStr := '';
          cmdstr := '';
...
//  addh := 0;
  ...
  if (GetTickCount > SurfaceRefTick) or (nShowIndex <> ShowIndex) then begin
    ...
    while True do begin
      if str = '' then break;
      if h >= (SCREENHEIGHT - 20) then begin
...
//              BColor := StrToIntDef(sBColor, Color);
  ...
  Result.Y := HintHeight;
  //HintX := 20;
  //HintY := (SCREENHEIGHT - HintHeight) div 2;
end;
...
{$IF Var_Interface = Var_Mir2}
    ax := 30;
{$ELSE}
    ax := (SCREENWIDTH - SayHint.SaySurface.Width - 20) div 2;
{$IFEND}
...
var
  ...
  dwTime: LongWord;
begin
  if nX + HWidth > SCREENWIDTH then hx := SCREENWIDTH - HWidth
  else hx := nX;
  if nY + HHeight > SCREENHEIGHT then hy := SCREENHEIGHT - HHeight
  else hy := nY;
  if hY < 0 then hy := 0;
  if hx < 0 then hx := 0;
