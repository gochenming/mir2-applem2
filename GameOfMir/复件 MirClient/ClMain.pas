...
uses
  ...
  JSocket, ExtCtrls, DrawScrn, HGEGUI, HGETextures, Imm, DES, HGE, HGESounds,  
  ...
  if (GetSpellPoint(pcm) <= g_MySelf.m_Abil.MP) or (pcm.Def.Magic.btEffectType = 0) then begin
    ...
    if pcm.Def.Magic.btEffectType = 0 then begin
     ...
     { else
      ...
      tdir := GetFlyDirection(390, 275, tx, ty);
...
begin
  if boReset then begin
    if FboDisplayChange then begin
      ...
      lpDevMode.dmPelsWidth := FDevMode.dmPelsWidth;
      lpDevMode.dmPelsHeight := FDevMode.dmPelsHeight;
      lpDevMode.dmDisplayFrequency := FDevMode.dmDisplayFrequency;
    ...
    if not FboDisplayChange then begin
      ...
      EnumDisplaySettings(nil, 0, lpDevMode);
      lpDevMode.dmFields := DM_PELSWIDTH Or DM_PELSHEIGHT Or DM_DISPLAYFREQUENCY;
      lpDevMode.dmPelsWidth := SCREENWIDTH;
      lpDevMode.dmPelsHeight := SCREENHEIGHT;
      lpDevMode.dmDisplayFrequency := FDevMode.dmDisplayFrequency;
      if ChangeDisplaySettings(lpDevMode, 0) = DISP_CHANGE_SUCCESSFUL then begin
